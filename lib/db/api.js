  ...
  function wrappedAdminLockGroup() {
    return db
      ...
      .then(function(stats) {
        ...
        else if (stats.skipped || stats.unchanged) {
          result.status = true
        }
        return result
      })
...
dbapi.saveSession = function(session) {
  return db.run(r.table('sessions').insert({
      id: session.id
    , email: session.email
    , createdAt: r.now()
    , expireAt: r.now().add(session.ttlSeconds)
    }))
}

dbapi.loadSession = function(id) {
  return db.run(r.table('sessions').get(id))
}

dbapi.deleteSession = function(id) {
  return db.run(r.table('sessions').get(id).delete())
}

dbapi.deleteUserSessions = function(email) {
  return db.run(r.table('sessions').getAll(email, {index: 'email'}).delete())
}
...
dbapi.setDeviceIosVersion = function(message) {
    return db.run(r.table('devices').get(message.id))
      .then(result => {
        db.run(r.table('devices').get(message.id).update({
...
dbapi.getDeviceGroupOwner = function(serial) {
  return db.run(r.table('devices').get(serial)('group')('owner'))
}
...
dbapi.initializeIosDeviceState = function(publicIp, message) {
  ...
  const data = {
    ...
    usage: null,
    ios: true,
    iosClientChannel: message.serial,
    display: {
  ...
  return db.run(r.table('devices').get(message.serial).update(data))
  .then(function(stats) {
    if (stats.skipped) {
      return dbapi.getRootGroup().then((group) => {
        ...
        return db.run(r.table('devices').insert(data))
          .then(() => {
          dbapi.addOriginGroupDevice(group, message.serial)
        })
      })
    }
