...
dbapi.saveDeviceIosInitialState = function(serial, device) {
  var data = {
    ...
    usage: null,
    ios: true,
    iosClientChannel: device.serial,
    display: {
      ...
      url: `ws://${device.wdaHost}:${device.screenPort}/`
    },
    group: {
      owner: {
        email: 'administrator@fakedomain.com',
...
dbapi.loadDevice = function(serial, groups) {
    ...
    return db.run(r.table('devices').get(message.id))
      .then(result => {
        db.run(r.table('devices').get(message.id).update({
            ...
            model: `${result.model} ${message.sdkVersion}`,
            provider: {
              name: `${result.provider.name} ${message.sdkVersion}`
            }
          }
        ))
      })
...
dbapi.initializeIosDeviceState = function(publicIp, message) {
  ...
  console.log('initializeIosDeviceState!!!')
  try {
