...
dbapi.setDeviceAbsent = function(serial) {
  console.log(' ===== SetDeviceAbsent =====',serial)
  return db.run(r.table('devices').get(serial).update({
    present: false
  , presenceChangedAt: r.now()
  }))
...
dbapi.setDeviceSocketPorts = function(data, publicIp) {
  return db.run(r.table('devices').get(data.serial).update({
      display: {
        url: `ws://localhost:${data.screenPort}/`
      },
...
dbapi.setAppBundleId = function(message) {
  return db.run(r.tabel('device').get(message.serial).update({
