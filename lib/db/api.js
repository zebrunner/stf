...
dbapi.deleteDisconnectedDevices = function() {
...
dbapi.setDeviceApp = function(message) {
  db.run(r.table('devices').get(message.serial))
    .then(result => {
      let index = result.installedApps.findIndex(item => {
        return (item.bundleName === message.bundleName && item.bundleId === message.bundleId)
      })
      if(index >= 0) {
        result.installedApps[index] = {
...
dbapi.getInstalledApplications = function(message) {
  return db.run(r.table('devices').get(message.serial)('installedApps'))
}
