...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('ios-provider-max-port', {
      describe: 'Highest port number for device workers to use.'
      , type: 'number'
      , default: 7900
    })
    .option('ios-provider-min-port', {
      describe: 'Lowest port number for device workers to use.'
      , type: 'number'
      , default: 7701
    })
    ...
    .option('wda-path', {
        describe: 'The path for WebDriverAgent directory'
      , type: 'string'
      , default: '/Users/macbook/Documents/"Qa Tools"/appium_WDA/WebDriverAgent.xcodeproj'
    })
    .option('udid-storage', {
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
       ...
       , procutil.fork(path.resolve(__dirname, '..'), [
          'ios-provider',
        , '--name', argv.providerIos,
        , '--min-port', argv.iosProviderMinPort
        , '--max-port', argv.iosProviderMaxPort
        , '--connect-app-dealer', argv.bindAppDealer
        , '--connect-dev-dealer', argv.bindDevDealer
        , '--connect-sub', argv.bindDevPub
        , '--connect-push', argv.bindDevPull
