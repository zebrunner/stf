      ...
      case FrameProducer.STATE_STOPPED:
        if (this.desiredState.next() === FrameProducer.STATE_STARTED) {
          ...
          .then(function(banner) {
            log.verbose('banner and _readFrames(this.socket.stream)')
            this.banner = banner
            return this._readFrames(this.socket.stream)
          })
          .then(function() {
            log.verbose('STATE_STARTED')
            this.runningState = FrameProducer.STATE_STARTED
            log.verbose('emit start message')
            this.emit('start')
          })
          .catch(Promise.CancellationError, function() {
            log.verbose('catch Promise.CancellationError')
            return this._stop()
          })
          .catch(function(err) {
            log.verbose('catch function(err)' + err)
          ...
          .finally(function() {
            log.verbose('finally _ensureState()')
            this._ensureState()
          })
        }
        else {
          log.verbose('setImmediate(this._ensureState.bind(this))')
