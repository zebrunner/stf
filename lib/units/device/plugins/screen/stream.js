          ...
          if (this.desiredState.next() === FrameProducer.STATE_STARTED) {
            ...
            this._startService().bind(this)
              ...
              .then(function() {
                ...
                console.log('call emit start , stream.js -------------->')
                this.emit('start')
              })
    ...
    return createServer()
      .then(function(wss) {
        ...
        frameProducer.on('start', function() {
          console.log('starting streaming frameProducer')
        ...
        wss.on('connection', function(ws) {
          ...
          function wsFrameNotifier(frame) {
            log.info('wsFrameNotifuer', frame)
            return send(frame, {
              binary: true
            })
          }
          ...
          ws.on('message', function(data) {
            console.log(' <========== Webscokets message ==========>  ')
