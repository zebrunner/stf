    ...
    return new Promise((resolve, reject) => {
      execShellCommand(linuxCommandQuery.join(";")).then(() => {
        parser(`${filepath}.xml`).then(data => {
          resolve(data);
        }).catch(err => {console.error('Unable to parse Info.plist(xml) from ipa/app/zip', err);})
      }).catch(err => {console.error('Unable to unpack ipa/app/zip and find Info.plst', err);})
    })
}
