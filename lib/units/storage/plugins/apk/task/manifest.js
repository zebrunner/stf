...
const appleReaderPromise = (filepath) => {
    ...
    const linuxCommandQuery = [
        ...
        `xmllint --valid ${tmpDirPath}/Info.plist && if [[ $? -eq 0 ]]; then cp ${tmpDirPath}/Info.plist ${filepath}.xml; else plistutil -i ${tmpDirPath}/Info.plist -o ${filepath}.xml; fi`,
        `rm -rf ${tmpDirPath}`
    ];
    ...
    return new Promise((resolve, reject) => {
      execShellCommand(linuxCommandQuery.join(";")).then(() => {
        parser(`${filepath}.xml`).then(data => {
          ...
          console.error('Unable to parse Info.plist(xml) from ipa/app/zip', err);
          reject(err)
        })
      }).catch(err => {
        console.error('Unable to unpack ipa/app/zip and find Info.plst', err);
        reject(err)
      })
    })
}
