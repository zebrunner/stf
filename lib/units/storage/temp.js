...
module.exports = options => {
  ...
  app.post('/s/uploadIos/:plugin/:deviceId', (req, res) => {
    ...
    let tmpDirPath = path.join(options.saveDir, `tmp/${tmpDirId}`)
      ...
      mkdir(tmpDirPath)
        .then(() => {
          ...
          form.parse(req, (err, field, files) => {
            if (!err) {
              log.info('no err during form.parse...')
              Object.keys(files).map((value) => {
                let file = files[value]
                log.info('starting decompressing...')
                decompress(file.path, tmpDirPath)
                  ...
                  .catch(() => {
                    log.info('catched decompress error!')
                    if (REG_EXPRESSION_ZIP_FILE.test(file.name)) {
                      res.status(201)
                      ...
                      let iosDeploy = spawn('ios-deploy5', args, {
                        cwd: tmpDirPath,
                        shell: true
                      })
                      ...
                      iosDeploy.stderr.on('data', data => {
                        log.info('iosDeploy.stderr data :', data.toString())
                      })

                      iosDeploy.on('close', code => {
        ...
        .catch(err => log.fata('Unable to create temp dir', err))
    }
    else {
      res.status(404)
      res.end()
