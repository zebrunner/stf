...
const urlsafety = require('../../util/urlsafety')
const storageAuth = require('./middleware/storage-auth')
...
module.exports = options => {
  ...
  // Require authentication for the storage routes. Internal services pass the
  // dedicated storage token header; browser users are validated via their
  // signed session cookie.
  const requireStorageAuth = storageAuth({
  ...
  Promise.map(options.endpoints.push, endpoint => {
    ...
    .catch(err => log.fata('Unable to connect temp to push endpoints', err))
  ...
  app.post('/s/download/:plugin', requireStorageAuth, (req, res) => {
    requtil
      .validate(req, () => req.checkBody('url').notEmpty())
      .then(() => urlsafety.assertSafeUrl(req.body.url))
      .then(() => download(req.body.url, {dir: options.cacheDir, ssrfProtect: true}))
      .then(file => {
        return {
  ...
  app.post('/s/upload/:plugin', requireStorageAuth, (req, res) => {
  ...
  app.get('/s/blob/:id/:name', requireStorageAuth, (req, res) => {
