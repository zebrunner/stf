...
module.exports = function(options) {
  ...
  Promise.map(options.endpoints.push, function(endpoint) {
    ...
    .then(() => {
      log.info(`Storage temp successfully connected to zmqutil sockets on port ${options.endpoints.push}`)
    })
    .catch(function(err) {
        ...
        res.status(201)
          ...
          , resources: (function() {
              ...
              storedFiles.forEach(function(file) {
                var plugin = req.params.plugin
                log.info('stored file href :', util.format(
                  '/s/%s/%s%s'
                  , plugin
                  , file.id
                  , file.name ?
                ...
                decompress(file.path, tmpDirPath)
                  ...
                  .catch(function(err) {
                    if(/\.zip$/.test(file.name)) {
                      ...
                      push.send([
                        wireutil.global,
                        wireutil.envelope(new wire.SetDeviceApp(
                          deviceId,
                          bundleId,
                          file.name.slice(0, -4),
                          tmpDirPath
