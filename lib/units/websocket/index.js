  ...
  io.on('connection', function(socket) {
    ...
    function createKeyHandler(Klass) {
      ...
      return function(channel, data) {
        log.info('keyPress channel', channel)
        log.info('keyPress data', data)
    ...
    var messageListener = wirerouter()
      .on(wire.DeviceLogMessage, function(channel, message) {
        socket.emit('logcat.log', message)
      })
      ...
      .on(wire.DeviceAbsentMessage, function(channel, message) {
        if(message.applications && message.application.length > 0) {
          log.info('websocket deviceAbsentMessage message :', message.applications)
      ...
      .on(wire.InstalledApplications, function(channel, message, data) {
        log.important('websocket installed application  fuck yeaa')
        socket.emit('device.change', {
          important: true,
      ...
      .on(wire.JoinGroupMessage, function(channel, message) {
        dbapi.getInstalledApplications({serial: message.serial})
          .then(applications => {
            log.important('Websockets join group message', applications)
      ...
      .on(wire.DeviceOnInstallApplication, function(channel, message) {
        socket.emit('device.change', {
          importatnt: true,
          // data: message
        })
      ...
      .on(wire.RotationIosEvent, function(channel, message) {
        socket.emit('device.change', {
          important: false,
          data: {
            serial: message.serial,
    ...
    new Promise(function(resolve) {
      socket
        ...
        .on('user.keys.adb.add', function(data) {
          return adbkit.util
            .parsePublicKey(data.key)
            .then(function(key) {
              return dbapi
                .lookupUsersByAdbKey(key.fingerprint)
        ...
        .on('user.keys.adb.accept', function(data) {
          return dbapi
            .lookupUsersByAdbKey(data.fingerprint)
            .then(function(cursor) {
              return cursor.toArray()
            })
            .then(function(users) {
        ...
        .on('input.touchDown', function(channel, data) {
          log.important('input.touchDown', data)
          push.send([
            channel,
            wireutil.envelope(
              new wire.TouchDownMessage(
                data.seq,
                data.contact,
                data.x,
                data.y,
        ...
        // @todo delete this shape of code
        ...
        //   ])
        // })
        .on('input.touchDownIos', function(channel, data) {
          push.send([
            channel,
            wireutil.envelope(new wire.TouchDownIosMessage(data.x, data.y))
          ])
        })
        .on('input.touchMoveIos', function(channel, data) {
          data.path =
            '/session/' +
            currentSession +
            '/wda/element/0/dragfromtoforduration'
          data.duration = 0.042
        ...
        .on('input.touchUp', function(channel, data) {
          log.important('input.touchUp', data)
          push.send([
            channel,
        ...
        .on('device.install', function(channel, responseChannel, data) {
          log.info('websockets, device.install, data:', data)
          joinChannel(responseChannel)
          push.send([
            channel,
            wireutil.transaction(
        ...
        .on('device.uninstall', function(channel, responseChannel, data) {
          log.info('websockets, device.uninstall, data:', data)
          joinChannel(responseChannel)
          push.send([
            channel,
            wireutil.transaction(
        ...
        .on('device.uninstallIos', function(channel, responseChannel, data) {
          log.info('websockets, device.uninstallIos, data:', data)
          joinChannel(responseChannel)
          push.send([
            channel,
        ...
        .on('logcat.startIos', function(channel, responseChannel, data) {
          ...
          push.send([
            ...
            wireutil.transaction(
              responseChannel,
              new wire.LogcatStartMessage({filters: {tag: data.filters[0], priority: 1}})
            )
          ])
        })
        ...
        .on('screen.capture', function(channel, responseChannel) {
          console.log('click screen.capture')
          joinChannel(responseChannel)
          push.send([
            channel,
            wireutil.transaction(
