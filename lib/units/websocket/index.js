...
module.exports = function(options) {
  ...
  var other_server = require('socket.io-client')('http://localhost:8110')
  var channelRouter = new events.EventEmitter()

  other_server.on('connection',function(sockt){
  ...
  .catch(function(err) {
    log.info('Unable to connect to push endpoint', err)
    lifecycle.fatal()
  })
  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.DeviceIntroductionMessage, function(channel, message) {
        log.info('deviceIntroductions Message!!!!!')
      ...
      .on(wire.DeviceAbsentMessage, function(channel, message) {
        log.info(' ===== websocket ===== func DeviceAbsentMessage')
      ...
      .on(wire.JoinGroupMessage, function(channel, message) {
        console.log(' Join Group Message ---- websocket')
        socket.emit('device.change', {
          important: true
      ...
      .on(wire.LeaveGroupMessage, function(channel, message) {
        console.log(' Leave Group Message ---- websocket')
        socket.emit('device.change', {
          important: true
        , data: datautil.applyOwner({
              serial: message.serial
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('input.touchDown', function(channel, data) {
          data.path = '/session/'+currentSession+'/wda/tap/0'
          log.info('output device channel ', channel)
          log.info('output touchDown data',data)
          io.to(client['somekey']).emit('touchDown',JSON.stringify(data))
          push.send([
            channel
          , wireutil.envelope(new wire.TouchDownIosMessage(
              data.x
            , data.y
            ))
          ])
        ...
        .on('input.touchMove', function(channel, data) {
          log.info('output touchMove data',data)
          // io.to(client['somekey']).emit('touchMove',JSON.stringify(data))
          push.send([
            channel
        ...
        .on('input.touchMoveIos2', function(channel, data) {
          data.path = '/session/'+currentSession+'/wda/element/0/dragfromtoforduration'
          data.duration = 0
          log.info('output touchMoveIos data',data)
          io.to(client['somekey']).emit('swipe',JSON.stringify(data))
          push.send([
            channel
            , wireutil.envelope(new wire.TouchMoveMessage(
               data.toX,
        ...
        .on('input.touchMoveIos', function(channel, data) {
         ...
         // io.to(client['somekey']).emit('swipe',JSON.stringify(data))
          push.send([
            channel
            , wireutil.envelope(new wire.TouchMoveIosMessage(
              data.toX,
              data.toY,
              data.fromX,
              data.fromY,
        ...
        .on('input.keyPress', function(data){
          data.path = '/session/'+currentSession+'/wda/homescreen'
          console.log(data, ' output input.keyPress')
          io.to(client['somekey']).emit('home',JSON.stringify({path: '/wda/homescreen'}))
        })
        ...
        .on('group.invite', function(channel, responseChannel, data) {
          console.log(' ====== group.invite websocket =====')
          console.log ('==== group invite ==== channel',channel)
          console.log ('==== group invite ==== responseChannel',responseChannel)
          console.log ('==== group invite ==== data',data)
          joinChannel(responseChannel)
          // socket.emit('start',{
          //   version: 1,
          //   length: 24,
        ...
        .on('connect.start', function(channel, responseChannel) {
          console.log(' handle sockets with emit connect.start',channel)
          console.log('output channales , func connect.start',channels)
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
        ...
        .on('openSettings', function(message) {
          console.log(message,' --------------------- message -----------------------')
          socket.emit('openSettings', message)
        })
        .on('manageOptions', function(message) {
          console.log(message,' --------------------- message -----------------------')
        })
        .on('connectIos', function(message) {
          client['somekey'] = socket.id
          currentSession = message
          console.log(message, 'connectIos')
         socket.emit('connectedToClient', 'some data')
        })
        .on('takeScreenShot', function(message) {
          console.log(message, ' --------------------- message -----------------------')
          socket.emit('takeScreenShot', message)
          console.log(socket,socket)
          io.to(client['somekey']).emit('takeScreenShot', message);
          //connectedIosClient.emit('takeScreenShot', message)
      })
    })
    .finally(function() {
