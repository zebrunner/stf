...
module.exports = function(options) {
  ...
  log.important('Output endpoints sub', options.endpoints.sub)
  log.important('Output endpoints push', options.endpoints.push)
  log.info('output provider options', options)
  // Output
  ...
  client.trackDevices().then(function(tracker) {
    ...
    log.info('output tracker', tracker)
    ...
    tracker.on('add', filterDevice(function(device) {
      ...
      log.info('-------------- add solo :', solo)
      log.info('add options :', options.name)
      log.info('add wireutil.global :', wireutil.global)
      ...
      function work() {
        return (worker = workers[device.id] = spawn())
          .then(function() {
            ...
            worker = null
            console.log(' -----  provider/index.js ------ work')
