...
module.exports = function(options) {
  ...
  log.info('start Ios Provider')
  // var client = Ios.createClient({
  //   host: options.iosHost,
  //   port: options.
  // })
  var workers = {}
  ...
  log.info('output options ', options)
  ...
  ;[solo].forEach(function(channel) {
    log.info(options.adbHost, 'adbHost options')
    log.info(options.adbPort, 'adbHost options')
  ...
  //     log.info('connected to port ' + 7400)
  //     var id = Math.random()
  //     clients[id] = ws
  //     log.info('новое соединение ' + id)
  //     var msg = {
  //       type: 'message',
  //       text: 'hi',
  //       date: Date.now()
  //     }
  //     clients[id].send('start ' + JSON.stringify(msg))
  ...
  io.on('connection', function(socket) {
    ...
    //   .on(wire.StartStreaming, function(data,channel) {
    //     log.info(' sub start streaming ---------')
    //   })
    // .handler())
    // devDealer.on('message', wirerouter()
    //   .on(wire.StartStreaming, function(data,channel) {
    //     log.info('devDealer start streaming =========')
    //   })
    //   .handler())
    wirerouter()
      .on(wire.StartStreaming, function(data,channel) {
    ...
    socket.on('sendImgData', function(msg) {
      for(var key in clients) {
        clients[key].send('start ' + JSON.stringify(msg))
      }
    })
    ...
    socket.on('addDevice', function(device) {
      ...
      log.info('device.width :', typeof device.width)
      log.info('device.height :', typeof device.height)
      push.send([
        wireutil.global
        , wireutil.envelope(new wire.SetIosDeviceDisplay(
          device.id,
          socket.id,
          device.width,
          device.height
      ...
      function spawn() {
        ...
        var didExit = false
        log.important('output device, spawn forker', device)
      ...
      function removeListener() {
        register.then(function() {
          ...
          delayedTotals()
          console.log(' -----  provider/index.js ------ removeListener')
          // Tell others the device is gone
          push.send([
            wireutil.global
            , wireutil.envelope(new wire.DeviceAbsentMessage(
