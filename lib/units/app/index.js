...
var iconMiddleware = require('./middleware/icon')
...
module.exports = function(options) {
  ...
  app.use('/static/app/icons', iconMiddleware(__dirname))
  ...
  app.use(function(req, res, next) {
    ...
    // Allow only same-origin framing so the app cannot be clickjacked (SLV-008).
    res.setHeader('X-Frame-Options', 'SAMEORIGIN')
    res.setHeader('Content-Security-Policy', "frame-ancestors 'self'")
    next()
  })

  // Terminate the server-side session and clear the cookie (SLV-004). Defined
  // before the auth gate so it works regardless of session validity, and before
  // csrf() so the logout button needs no token.
  app.post('/app/api/v1/logout', function(req, res) {
    authsession.revoke(req.session)
      .catch(function() {})
      .then(function() {
        req.session = null
