...
function removeDevice(serial, req, res) {
  ...
  function deleteGroupDevice(id) {
    const lock = {}

    return dbapi.adminLockGroup(id, lock).then(function(stats) {
      ...
      if (!result.status) {
        return dbapi.getGroup(id).then(function(group) {
          ...
          if (!result.replaced) {
            return apiutil.lightComputeStats(res, result)
          }
          group = lock.group = result.changes[0].new_val
