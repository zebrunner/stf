...
function removeDevice(serial, req, res) {
  ...
  function deleteGroupDevice(groupId) {
    return dbapi.adminLockGroup(groupId, lock).then(lockingSuccessed => {
      if (!lockingSuccessed) {
        apiutil.respond(res, 500, "Group is busy. Try again later.")
      }

      return dbapi.getGroup(groupId).then(group => {
        lock.group = group;
    ...
    .finally(function() {
      dbapi.adminUnlockGroup({group: {id: groupId}})
    })
  }
    ...
    function wrappedDeleteDeviceInDatabase() {
      ...
      return dbapi.loadDeviceBySerial(serial).then(function(device) {
        if (device && device.group.id === device.group.origin) {
          return deleteGroupDevice(device.group.id)
            .then(function(group) {
              if (group !== 'not found') {
  ...
  return dbapi.lockDeviceByOrigin(req.user.groups.subscribed, serial).then(function(stats) {
    ...
    return dbapi.getDeviceTransientGroups(serial).then(function(groups) {
      ...
      return Promise.each(groups, function(group) {
        return deleteGroupDevice(group.id)
      })
      .then(function() {
        if (!groups.length && !anyBookingState && bookingState) {
          return 'unchanged'
