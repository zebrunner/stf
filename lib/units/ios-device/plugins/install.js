...
const installApp = (udid, filepath, isSimulator) => {
    const simulatorCommands = [
        `cd ${filepath} && unzip * && xcrun simctl install ${udid} *.app`,
    ];

    const deviceCommands = [
        `ios install --path=${filepath} --udid=${udid}`,
    ];
            ...
            response.pipe(file)
             ...
             file.on("finish", () => {
                 ...
                 return promiseutil.periodicNotify(
                   installApp(options.serial, tmpDirPath, isSimulator)
                   , 250
                 )
                 .progressed(function() {
