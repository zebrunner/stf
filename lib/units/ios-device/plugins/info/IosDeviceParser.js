...
//-----------------------------------------------------------------------------------------------
/**
 * Call `http://wdaHost:wdaPort/wda/device/info` to fetch extra device info
 * @param {string} deviceId @description identifier e.g. 'A311989D-81CA-49C7-BB88-FDDC48376A71'
 * @constructor
 */
const WdaInfo = function(deviceId) {
  this.parsedData = null
  this.deviceId = deviceId

  const getDeviceDataById = (deviceId) => {
    const defaultArgs = ['']
    //TODO: use curl and read wda host/port from env vars!
    const iosWdaUrl = spawn('ls', defaultArgs)

    return new Promise((resolve, reject) => {
      iosWdaUrl.stdout.on('data', (data) => {
...
const IosDeploy = function(deviceId) {
  ...
  const getDeviceDataById = (deviceId) => {
    const defaultArgs = ['-c']
    const iosDeployProcess = spawn('ios-deploy4', defaultArgs)
