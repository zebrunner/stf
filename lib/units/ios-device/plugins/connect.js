...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group, solo, urlformat) {
    ...
    //     console.log('Trying ConnectStartMessage with channel',channel)
    ...
    //       })
    //       .catch(function(err) {
    //         log.error('Failed to stop connect servicePreparing device', err.stack)
    //         push.send([
    //           channel
    //           , reply.fail(err.message)
    //         ])
    ...
    function createServer() {
      log.info('Starting WebSocket server on port %d', options.screenPort)
