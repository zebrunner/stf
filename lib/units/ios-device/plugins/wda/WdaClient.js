  ...
  .define((options, notifier, push) => {
    ...
    const WdaClient = {
      ...
      typeKey: function(params) {
        ...
        this.keyboardActions.push({type: "keyDown", value: params.value[0]})
        this.keyboardActions.push({type: "keyUp", value: params.value[0]})

        //TODO: organize smart final actions flush if delay between commands more then 250ms for example
        if (params.value[0] == ' ') {
      ...
      getSwipeActions: function(params) {
        return {
          actions: [
            {
              type: 'pointer',
              ...
              actions: [
                ...
                {type: 'pointerMove', duration: params.duration*1000, x: params.toX, y: params.toY},
                {type: 'pointerUp'}
              ]
            }
          ],
      ...
      swipe: function(params) {
        ...
        return this.handleRequest({
          method: 'POST',
          uri: `${this.baseUrl}/session/${this.sessionId}/actions`,
          body: this.getSwipeActions(scale),
          json: true
        })
      },
      getTapActions: function(params) {
        return {
          actions: [
            {
              type: 'pointer',
        ...
        if(!this.isMove) {
          ...
          if(((new Date()).getTime() - this.tapStartAt) <= 1000 || !this.tapStartAt) {
            return this.handleRequest({
              method: 'POST',
              uri: `${this.baseUrl}/session/${this.sessionId}/actions`,
              body: this.getTapActions(params),
              json: true
            })
          } else {
            return this.handleRequest({
