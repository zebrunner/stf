  ...
  .define((options, notifier, push) => {
    ...
    const WdaClient = {
        ...
        return this.handleRequest({
          ...
          .then((statusResponse) => {
            ...
            if (statusResponse.sessionId) {
              ...
              if (this.sessionId === this.sessionStopping) {
                log.error('Trying to reuse session for which "stopSession" was called')
              }
      ...
      stopSession: function() {
        ...
        this.sessionStopping = this.sessionId
          ...
          actions: [
            {
              ...
              actions: [
                ...
                {type: 'pointerMove', duration: scale.duration * 1000, x: scale.toX, y: scale.toY},
                {type: 'pointerUp'},
              ],
            },
          ],
          ...
          .then((batteryInfoResponse) => {
            ...
            push.send([
              ...
              wireutil.envelope(new wire.BatteryIosEvent(
                ...
                'usb',
                status,
                parseInt(batteryInfoResponse.value.level * 100, 10), // TODO: incorrect uses of parseInt
                'n/a',
                100,
              ))
            ])
      ...
      pressPower: function() {
        ...
        return this.handleRequest({
          ...
          .then(response => {
            const lockPath = response.value ? 'unlock' : 'lock'
            const url = `${this.baseUrl}/session/${this.sessionId}/wda/${lockPath}`
