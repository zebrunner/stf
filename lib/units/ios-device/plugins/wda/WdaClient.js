  ...
  .define((options, notifier, push) => {
    ...
    const WdaClient = {
      ...
      connect: function () {
        ...
        return new Promise((resolve, reject) => {
          request.get(`${this.baseUrl}/status`)
            .then(response => {
              // First time we have no session and it's null in the response
              let gotSessionInResponse = false
              try {
                this.sessionId = JSON.parse(response).sessionId
                if (this.sessionId) {
                  gotSessionInResponse = true
              ...
              //If get no session try to make it manifestly
              if (!gotSessionInResponse) {
                ...
                return this.handleRequest({
                  ...
                  .then(sessionResponse => {
                    try {
                      this.sessionId = JSON.parse(sessionResponse).sessionId
                      log.info('sessionId: ' + this.sessionId)
                    } catch (e) {
                      log.error('Failed to parse json object', e)
                      reject(e)
      ...
      initializeDeviceData: function() {
        ...
        return new Promise((resolve, reject) => {
          try {
            this.connect() // to init sessionId to read device data using WDA
            this.size()
            .then(response => {
