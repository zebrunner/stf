...
module.exports = syrup.serial()
  ...
  .define((options, notifier, push) => {
    ...
    socket.on('connect', () => {
      console.log(`Connected to wda ${options.wdaHost}:${options.wdaPort} `);

    //TODO: open in valid editor and apply formatting
    const WdaClient = {
...
*/
  ...
  return WdaClient
    ...
    socket.on('close', () => {
      lifecycle.fatal(`Disconnected from wda ${options.wdaHost}:${options.wdaPort}`) // exit with error code 1 is the best way to activate valid auto-healing steps with container(s) restart
    });
})
