...
*/
  ...
  function connectToWdaMjpeg() {
    ...
    */
    ...
    socket.on('connect', () => {
      console.log(`Connected to wda mjpeg ${options.wdaHost}:${options.connectPort} `);
    });

    socket.on("close", function() {
      log.error(`wda mjpeg connection is lost`)
      //TODO: initiate wda restart somehow
      socket.connect(options.connectPort, options.wdaHost);
      //notifier.setDeviceAbsent(`wda mjpeg connection is lost`)
      //lifecycle.fatal(`wda mjpeg connection is lost`)
    });
  ...
  function connectToWda() {
    /*
     * WDA PORT connection is stable enough to be track status wda server itself. But it is closed as only we do stop usage so need to reconnect. 
     * If reconnect fail - need restart wda and stf.
    */
    let socket = new net.Socket();
    socket.connect(options.wdaPort, options.wdaHost);

    socket.on('connect', () => {
      console.log(`Connected to wda port ${options.wdaHost}:${options.wdaPort} `);
    });

    socket.on("close", function() {
      log.error(`wda connection is lost`)
      connectToWda()
      //notifier.setDeviceAbsent(`wda mjpeg connection is lost`)
      //lifecycle.fatal(`wda mjpeg connection is lost`)
    });
