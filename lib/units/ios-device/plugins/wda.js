...
module.exports = syrup.serial()
  ...
  .define((options, solo, router, push, sub, wdaClient) => {
    ...
    try {
      const Wda = {}
      let blockNextTouchUp = false
      let touchUpAt = getTimestamp()
            ...
            .then(response => {
              ...
              sub.on('message', wirerouter()
                ...
                .on(wire.TouchUpMessage, (channel, message) => {
                  // const touchUpTimeDiff = getTimestamp() - touchUpAt
                  // if(touchUpTimeDiff >= 0 && touchUpTimeDiff <= 500) {
                  //   wdaClient.doubleClick(deviceSize)
                  //   blockNextTouchUp = true
                  // } else if(!blockNextTouchUp) {
                  //   blockNextTouchUp = false
                  //   wdaClient.touchUp(deviceSize)
                  // }
                  wdaClient.touchUp(deviceSize)
                  touchUpAt = getTimestamp()
                })
