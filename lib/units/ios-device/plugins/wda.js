            ...
            .then(response => {
              ...
              sub.on('message', wirerouter()
                .on(wire.KeyPressMessage, (channel, message) => {
                  if(message.key === 'camera' || message.key === 'search'){
                    wdaClient.appActivate(iosutil.appActivate(message.key))
                  } else if (message.key === 'mute'){
                      let i
                      for( i = 0; i < 17; i++ ){
                        wdaClient.pressButton(iosutil.pressButton(message.key))
                      }
                  } else {
                      wdaClient.pressButton(iosutil.pressButton(message.key))
                  }
                ...
                .on(wire.StoreOpenMessage, (channel, message) => {
                  let store = 'com.apple.AppStore'
                  wdaClient.appActivate(store)
                })
