...
//      log.important('options: ', options)
      Wda.connect = () => {
	  const wdaHost = options.wdaHost
          const wdaPort = options.wdaPort
          log.important('wdaHost: ', wdaHost)
	  log.important('wdaPort: ', wdaPort)
      ...
      Wda.startStream = () => {
        log.info(`start WDA streaming on host ${wdaHost} and port ${wdaPort}`)
        return request.get('http://${wdaHost}:${wdaPort}')
      }
