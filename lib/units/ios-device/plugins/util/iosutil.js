...
let iosutil = {
  ...
  checkDevice: function(options, udid, log) {
    let localFile = fs.readFileSync(path.dirname(require.main.filename).replace('lib/cli', 'iosdevice.json')) // path to file where save devices
    let localDevices = JSON.parse(localFile)
      if(localDevices[udid]) {
        return udid
      }
      else if(options.udidStorage !== 'false' && JSON.parse(fs.readFileSync(options.udidStorage))[udid]) {
          return udid
      }
      else {
  ...
  getUri: function(options, port, log) {
    ...
    if(options.iproxy) {
      ...
      let localFile = fs.readFileSync(path.dirname(require.main.filename).replace('lib/cli', 'iosdevice.json'))
      let localDevices = JSON.parse(localFile)
