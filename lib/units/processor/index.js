...
module.exports = db.ensureConnectivity(function(options) {
  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.DevicePresentMessage, function(channel, message, data) {
      log.info('DevicePresentMessage data:', data)
      log.info('DevicePresentMessage data:', message)
    ...
    .on(wire.SetIosDevicePorts, function(channel, message, data) {
      log.info('SetIosDevicePorts message', message)
      dbapi.setDeviceSocketPorts(message, options.publicIp)
      .then(function(device) {
        log.info('SetIosDevicePorts message ====> then ', message)
        log.info(' setIosDevicePorts ,output device', device)
        devDealer.send([
    ...
    .on(wire.CheckIosDeviceConnected, function(channel, message) {
      log.info('CheckIosDeviceConnected message :', message)
      log.info('CheckIosDeviceConnected message :', message)
      dbapi.checkIosDeviceConnected(message)
        .then(condition => {
          if(!condition && condition === null) {
            log.info('checkIosDeviceConnected condition', condition)
            devDealer.send([
              message.channel
    ...
    .on(wire.DeleteDisconnectedDevices, function(channel, message) {
      log.info('deleteDisconnected Devices')
      dbapi.deleteDisconnectedDevices()
    })
    .on(wire.SetAppBundleId, function(channel, message) {
      log.info('wire Set app bundle id', message)
      dbapi.setAppBundleId(message)
    })
    .handler())
