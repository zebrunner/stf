...
module.exports = db.ensureConnectivity(function(options) {
  ...
  Promise.map(options.endpoints.push, function(endpoint) {
    return srv.resolve(endpoint).then(function(records) {
      return srv.attempt(records, function(record) {
        log.info('Processor sending output to "%s"', record.url)
    ...
    .catch(function(err) {
      log.fatal('Unable to connect processor to push endpoints', err)
    })
  ...
  devDealer.on('message', wirerouter()
    ...
    // introductionMessage for ios device
    .on(wire.DeviceIosIntroductionMessage, function(channel, message, data) {
      dbapi.saveDeviceIosInitialState(message.serial, message)
        .then(function() {
            devDealer.send([
              message.provider.channel
              , wireutil.envelope(new wire.DeviceRegisteredMessage(
    ...
    .on(wire.DeviceOnInstAppMessage, function(channel, message) {
      log.important('DeviceOnInstAppMessage', message)
    })
    // Worker messages
    ...
    .on(wire.SetIosDeviceDisplay, function(channel, message, data) {
      dbapi.setDeviceSocketDisplay(message)
    ...
    .on(wire.SetIosDevicePorts, function(channel, message, data) {
      dbapi.setDeviceSocketPorts(message, options.publicIp)
      .then(function(device) {
        devDealer.send([
          device.provider.channel
          , wireutil.envelope(new wire.StartStreaming(
            message.connectPort,
            device.iosClientChannel
          ))
        ])
        appDealer.send([channel, data])
      })
    })
    // @TODO DELETE GetInstalledApplications
    // .on(wire.GetInstalledApplications, function(channel, message) {
    //     dbapi.getInstalledApplications(message)
    //       .then(applications => {
    //         push.send([
    //           wireutil.global,
    //           wireutil.envelope(new wire.InstalledApplications(
    //             message.serial,
    //             applications
    //           ))
    //         ])
    //       })
    //       .catch(err => {
    //         log.error(`Failed to show installed applications : ${err}`)
    //       })
    // })
    .on(wire.InstalledApplications, function(channel, message) {
      log.info('InstalledApplication processor service ', message)
    })
    .on(wire.CheckIosDeviceConnected, function(channel, message) {
      dbapi.checkIosDeviceConnected(message)
        .then(condition => {
          if(condition === null || condition.present === false) {
            devDealer.send([
              message.channel
              , wireutil.envelope(new wire.ConnectDeviceViaUSB(
                message.id
              ))
            ])
          }
    ...
    .on(wire.DeleteDisconnectedDevices, function(channel, message) {
      dbapi.deleteDisconnectedDevices()
    })
    .on(wire.SetDeviceApp, function(channel, message) {
      log.info('wire.SetDeviceApp')
      dbapi.setDeviceApp(message)
    })
    .handler())
