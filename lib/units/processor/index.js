...
module.exports = db.ensureConnectivity(function(options) {
  ...
  appDealer.on('message', function(channel, data) {
    log.important('AppDealer messages ', data.toString())
  ...
  devDealer.on(
    ...
    wirerouter()
      ...
      .on(wire.DeviceIosIntroductionMessage, function(channel, message, data) {
        dbapi.saveDeviceIosInitialState(message.serial, message).then(function() {
          devDealer.send([
            message.provider.channel,
            wireutil.envelope(new wire.DeviceRegisteredMessage(message.serial)),
          ])
        ...
        dbapi
          ...
          .then(installedApps => {
            push.send([
              wireutil.global,
              wireutil.envelope(
                new wire.InstalledApplications(message.serial, wireutil.toInstalledApps(installedApps))
              ),
            ])
          })
          .catch(err => {
          ...
          .then(function(user) {
            if (user) {
              ...
              appDealer.send([
                ...
                wireutil.envelope(
                  new wire.JoinGroupByAdbFingerprintMessage(message.serial, message.fingerprint, message.comment)
                ),
              ])
            }
          })
          ...
          .then(function(user) {
            if (user) {
              devDealer.send([
                channel,
                wireutil.envelope(
                  new wire.AutoGroupMessage(new wire.OwnerMessage(user.email, user.name, user.group), message.response)
                ),
              ])
            } else if (message.currentGroup) {
              appDealer.send([
      ...
      .on(wire.SetIosDevicePorts, function(channel, message, data) {
        dbapi.setDeviceSocketPorts(message, options.publicIp).then(function(device) {
          devDealer.send([
            device.provider.channel,
            wireutil.envelope(new wire.StartStreaming(message.connectPort, device.iosClientChannel)),
          ])
          appDealer.send([channel, data])
        })
      })
        ...
        dbapi
          ...
          .then(condition => {
            if (condition === null || condition.present === false) {
              devDealer.send([message.channel, wireutil.envelope(new wire.ConnectDeviceViaUSB(message.id))]);
            }
          })
          .catch(err => {
            log.info(err)
      ...
      .on(wire.SizeIsoDevice, function(channel, message, data) {
        dbapi
          .sizeIsoDevice(message)
          .then(result => {
            log.info(result)
          })
          .catch(err => {
