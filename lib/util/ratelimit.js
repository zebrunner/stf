/**
* Brute-force guard for authentication endpoints (SLV-005).
*
* Counts failed login responses per client IP and per account id, slows the
* response as failures grow, and locks a key out with HTTP 429 once it passes the
* threshold. State is process-local, which suits STF's single auth unit per
* provider; a horizontally-scaled auth tier needs a shared store instead.
*/
var DEFAULTS = {
  windowMs: 3 * 60 * 1000
, maxAttempts: 5
...
// envInt reads a positive integer env var, optionally treating it as seconds.
...
// clientIp prefers the left-most X-Forwarded-For entry set by the edge proxy and
// falls back to the socket address. The account key below is the spoof-resistant
// dimension; this one is best-effort.
...
// accountId is the identifier the caller is trying to authenticate as.
...
module.exports = function(options) {
  ...
  // registerFailure grows the failure count and, past the threshold, locks the
  // key with an exponential back-off keyed on how many times it has locked.
  ...
  // delayFor returns the throttle applied to a failing response once failures
  // pass delayAfter, so automated guessing slows down before a hard lock.
