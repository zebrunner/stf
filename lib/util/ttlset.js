...
TtlSet.prototype.bump = function(value, time, flags) {
  ...
  console.log('ttlset , func bump  , output item =========>', item)
  ...
  console.log('output ttlset item --->', item)
}
...
TtlSet.prototype._drop = function(item, flags) {
  console.log('ttsset --------------->', item)
  if (item) {
    this._remove(item)

    delete this.mapping[item.value]
    console.log('ttlset _drom flags:', flags)
    console.log('ttlset _drom TtlSet.Silent:', TtlSet.SILENT)
    if ((flags & TtlSet.SILENT) !== TtlSet.SILENT) {
